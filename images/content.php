...
<div class="block-content-dark">
    <div class="details-main-block ">
        <div class="wrap">
            ...
            <?php  //$prev_id = $content->get_previous_article($content->content_id);
			    $content_prev 	= new content($prev_id);
				$title    = (strlen($content_prev->title)>31) ? substr($content_prev->title,0, 31).'...': $content_prev->title;
				$member1	= new member($content_prev->content_author);
			 ?>
              <div class="content-block-wrap">
                    <div class="img-wrap">
                        <a href="" title="">
                            <img src="<?php echo URI_CONTENT. $content_prev->content_thumbnail?>" alt="">        </a>
                    </div>
                    <div class="bot-block">
                        <div class="bot-block-in1">
                            <a href="" title=""><?php echo functions::deformat_string($title) ?></a>
                        </div>
                        <div class="bot-block-in2">
                            <p><?php echo functions::deformat_string($member1->first_name) .' '. strtoupper(substr($member1->last_name, 0, 1)) ?></p>
                            <p><?php echo date('m.d.Y', strtotime($content->created_date)) ?></p>
                        </div>
                    </div>
         ...
         <?php  //$next_id = $content->get_next_article($content->content_id);
				...
				$title1    = (strlen($content_next->title)>31) ? substr($content_next->title,0, 31).'...': $content_next->title;
        ...
        <div class="center-content-block">
            ...
            <div class="content-block article-block-big">
                    <div class="img-wrap"><img src="<?php echo URI_CONTENT. $content->content_thumbnail?>" alt=""></div>                    <div class="bot-block">
                        <div class="bot-block-in1" ><?php echo functions::deformat_string($title) ?></div>
                        <div class="bot-block-in2">
                            <?php $member1	= new member($content->content_author); ?>
                            <p><?php echo functions::deformat_string($member1->first_name) .' '. functions::deformat_string($member1->last_name) ?></p>
                ...
                <div class="content-block-wrap">
                    <div class="img-wrap">
                        <a href="<?php echo URI_ROOT.'contents/'.$content_next->seo_url1 ?>" title="">
                            <img src="<?php echo URI_CONTENT. $content_next->content_thumbnail?>" alt="">        </a>
                    </div>
                    <div class="bot-block">
            ...
            <div class="content-block-all content-block-details">
				...
				?>
      			...
      			<div class="block-tags" style="color: #48525C; text-transform: uppercase;"><span>Tags:</span>
               		 <!--<a href="<?php echo URI_ROOT ?>contents/search?tag=advice">ADVICE</a>-->
                     <?php 
					 	//print_r($tag_array);
						for($i =0 ; $i < count($tag_array); $i++)
						{  ?>
							<a href="<?php echo URI_ROOT ?>contents/search?tag=<?php echo $tag_array[$i]->tag ?>"><?php echo $tag_array[$i]->tag ?></a>	
					<?php	}
					  ?>
                </div>
                <?php } ?>
...
<!--<div class="fav" id="favorite_<?php echo $content->content_id ?>">
     <?php echo content_like::get_like_total($content->content_id, 'favorite'); ?>
                ...
                <div class="active activity-block plus" ><div class="count"><span class="like-count" id="cnt_like_<?php echo $content->content_id ?>"> <?php echo content_like::get_like_total($content->content_id, 'like'); ?></span></div><a style="cursor:pointer;" class="comment1" id="like_<?php echo $content->content_id ?>"></a></div>    
...
<div class="block-content-light">
    <div class="wrap">
        <div class="block-content-in">
           ...
           <span class="span_comment_sign_up">TO LEAVE A COMMENT, <a style="cursor:pointer;" class="link_comment_sign_up" id="login_btn"  data-reveal-id="loginBox" data-animation="fadeAndPop">SIGN UP</a> NOW</span>
           <?php }  else { ?>
           		
               <div class="block-comment block-comment-white block-comment-write">
                        <form autocomplete="off" class="add_comment_form" id="comments-form" action="#" method="post">
                                ...
                                <td>
                                    <div class="block-comment-gray">
                                        ...
                                        <p class="label text-3">
                                            ...
                                            <textarea size="20" maxlength="255" id="text-com" name="ContentComments[content]"></textarea>                                            <button class="but-big but-red" onclick="return validate_comment()">Submit
                                            </button>
                                        </p>
                                                                            </div>
                                </td>
		   ...
		   if(content_comment::get_comment_total($content->content_id) > 0)
		   {
		   	   ?>
                ...
                </div></div>--><!--<div class="keys" style="display:none" title="/contents/1024"><span>113</span></div>-->
