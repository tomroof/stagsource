...
class UploadHandler
{
	...
	function add_img($whichimg, $title, $category_id)  
	{  
		$add_to_db = $this->query("INSERT INTO gallery (image, title, category_id) VALUES
			 ('".$whichimg."','".$title."','". $category_id. "')") or die(mysql_error());  
		return $add_to_db;  
	}
	
	function delete_img($delimg)  
	{  
		$delete_from_db = $this->query("DELETE FROM gallery WHERE image = '$delimg'") or die(mysql_error());  
