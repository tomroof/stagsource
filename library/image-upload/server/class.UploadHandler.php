...
class UploadHandler
{
    ...
    function __construct($upload_folder = '', $min_width=1, $min_height=1, $options = null, $initialize = true, $error_messages = null) {
	   
	    $this->upload_folder1 = $upload_folder;
		//$this->category_id 	  = $_SESSION['cat_id'];
		
		$dir_upload		 = 'DIR_'.strtoupper($upload_folder);
		$uri_upload		 = 'URI_'.strtoupper($upload_folder);
...
/*			'database' => 'image_upload',  
           ...
           // 'upload_dir' => DIR_ROOT. $upload_folder.'/', //dirname($this->get_server_var('SCRIPT_FILENAME')).'/files/', 'userfiles/'
           // 'upload_url' => URI_ROOT. $upload_folder.'/', //$this->get_full_url().'/files/',
			'upload_dir' => constant($dir_upload), 
            'upload_url' => constant($uri_upload), 
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files', //'files',
    ...
    protected function set_additional_file_properties($file) {
		...
		$file->deleteUrl = $this->options['script_url']
            ...
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
            .'='.rawurlencode($file->name).'&folder='.$this->upload_folder1;
    ...
    protected function validate($uploaded_file, $file, $error, $index) {
        ...
        if (is_int($img_width)) {
            ...
            if ($this->options['min_width'] && $img_width < $this->options['min_width']) {
                $file->error = $this->get_error_message('min_width').'. Min width:'.$this->options['min_width'].'px';
                return false;
            }
    ...
    protected function get_unique_filename($name,$type = null, $index = null, $content_range = null) {
		...
		$name	= str_replace($default, '_', $name);
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
             //$category_id	= $this->handle_form_data($file, $index);
			//$title = $this->handle_form_title($file, $index);  //my 
            $upload_dir = $this->get_upload_path();
            if (!is_dir($upload_dir)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
                ...
                if ($append_file) {
					...
					//$file->upload_to_db = $this->add_img($file->name, $title, $category_id);    //my code
	...
	//public function add_img($whichimg, $title, $category_id)  
	public function add_img($whichimg)  
	{  
	    //$order_id	= $this->get_max_order_id() + 1;
		//$add_to_db = $this->query("INSERT INTO $this->upload_folder1 (image_category_id, title, image_name, status, order_id, register_date) VALUES
		//	 ('". $category_id. "','".$title."','".$whichimg."','Y', '". $order_id."', NOW())") or die(mysql_error());  
		//return $add_to_db;  
	}
	...
	{ 
		//$delete_from_db = $this->query("DELETE FROM $this->upload_folder1 WHERE image_name = '$delimg'") or die(mysql_error());  
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
				...
				//$unique_number	= $timestamp;
			
			
                $files[] = $this->handle_file_upload(
                    $upload['tmp_name'][$index],
                    $file_name ? $unique_number.$file_name : $unique_number.$upload['name'][$index], //$file_name ? $file_name : $upload['name'][$index],
    ...
    public function delete($print_response = true) {
        ...
        foreach($file_names as $file_name) {
			...
			$this->delete_img($file_name); // my code
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
