...
class UploadForm
{
    ...
    data-download-template-id="template-download-<?php echo $form_number ?>">
		...
		<?php 
			...
			$this->createUploadListing($cancel,$form_number);
	...
	public function createSingleForm($multiple = true, $progress= true, $start= true, $cancel = true, $delete = true, $edit= 0)
	{
		...
		*/
        ...
        <div class="row fileupload-buttonbar">
            <div class="col-lg-7">
                <!-- The fileinput-button span is used to style the file input field as button   -->
                <span class="btn btn-success fileinput-button <?php echo ($multiple) ? 'btn_admin' : 'btn_admin1'; ?>">
                    <i class="glyphicon glyphicon-plus"></i>
                    ...
                    <input type="file" name="files[]" <?php if($multiple) { echo 'multiple="true"'; } ?>  id="up_file" />  <!-- accept="image/*" <?php if($edit == 1) { echo 'disabled="disabled"'; } ?> --> 
                </span>
				
                
				<?php
				if($start) { ?>
                <button type="submit" class="btn btn-primary start btn_admin">
                    <i class="glyphicon glyphicon-upload"></i>
                    <span>Start upload</span>
                </button>
                ...
                <button type="reset" class="btn btn-warning cancel btn_admin">
                    <i class="glyphicon glyphicon-ban-circle"></i>
                    <span>Cancel upload</span>
                </button>
				<?php
				...
				if($delete) { ?>
                <button type="button" class="btn btn-danger delete btn_admin" >
                    <i class="glyphicon glyphicon-trash"></i>
                    <span>Delete</span>
                </button>
                <input type="checkbox" class="toggle" >
				<?php } else {?>
                <!--<input type="checkbox" class="toggle" disabled="disabled">-->
				<?php } ?>
		...
		<?php 
			//The template to display files available for upload
			$this->createUploadListing($cancel);
	...
	//The template to display files available for upload 
	public function createUploadListing($cancel, $form_number = '')
	{
		/*
			...
			<tr class="template-upload fade">
				...
				<td>
					{% if (!o.files.error && !i && !o.options.autoUpload) { %}
						<button class="btn btn-primary start btn_admin">
							<i class="glyphicon glyphicon-upload"></i>
							<span>Start</span>
						</button>
					{% } %}
					...
					<?php if($cancel) { ?>
					{% if (!i) { %}
						<button class="btn btn-warning cancel btn_admin">
							<i class="glyphicon glyphicon-ban-circle"></i>
							<span>Cancel</span>
						</button>
					{% } %}
		...
		*/
		...
		{%
		 for (var i=0, file; file=o.files[i]; i++) { 
		 	//alert(file.name);
		%}
			<tr class="template-download fade" id="temp-down{%= (i+1) %}">
				...
				<td>
					<?php if($delete) { ?>
					<button class="btn btn-danger delete btn_admin" data-type="{%=file.deleteType%}" data-url="{%=file.deleteUrl%}"{% if (file.deleteWithCredentials) { %} data-xhr-fields='{"withCredentials":true}'{% } %}>
						<i class="glyphicon glyphicon-trash"></i>
						<span>Delete</span>
					</button>
