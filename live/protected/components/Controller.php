<?php
/**
 * Controller is the customized base controller class.
 * All controller classes for this application should extend from this base class.
 */
class Controller extends CController
{
	/**
	 * @var string the default layout for the controller view. Defaults to '//layouts/column1',
	 * meaning using a single column layout. See 'protected/views/layouts/column1.php'.
	 */
	public $layout='//layouts/column1';
	/**
	 * @var array context menu items. This property will be assigned to {@link CMenu::items}.
	 */
	public $menu=array();
    public $seoPack = array();
	/**
	 * @var array the breadcrumbs of the current page. The value of this property will
	 * be assigned to {@link CBreadcrumbs::links}. Please refer to {@link CBreadcrumbs::links}
	 * for more details on how to specify this property.
	 */
	public $breadcrumbs=array();
        public $login_form;
        
                public function init() {
            parent::init();
            
            $this->login_form = new LoginForm();
        }
    protected function beforeRender($view)
    {
        if (Yii::app()->user->hasFlash('message')) {
            PopupFancy::showMessage('
				<div class="block-popup pass-request popup-notification">
                    <div class="block-popup-in">
                        <div class="title-popup-in">
                            <h2>Notification</h2>
                        </div>
                        <div class="block-popup-in-2">
							<div class="block-popup-in-3">
								<p>' . Yii::app()->user->getFlash('message') . '</p>
								<a href="javascript:;" class="but-big but-green uibutton" onclick="$.fancybox.close()">OK</a>
							</div>
						</div>
					</div>
                </div>
			', array('hideOnOverlayClick' => 'true',"autoDimensions"=>"false", "width" =>608));
        }

        Yii::import('application.modules.seopack.components.SeopackWidget');
        $SeopackWidget = new SeopackWidget();
        $SeopackWidget->run();
        return parent::beforeRender($view) ;
    }
}