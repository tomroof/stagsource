// JavaScript Document
function open_content_poll_item_details(content_poll_item_id,divId, show_edit_comment, comment_id, serial_value)
{  
	var ajaxrequest;
	...
	if(document.getElementById("show_content_poll_item_id").value==0) {  document.getElementById("show_content_poll_item_id").value=content_poll_item_id;}
	var bid		= document.getElementById("show_content_poll_item_id").value;
	//alert(bid+"   "+content_poll_item_id);
	if(show=='0' && bid==content_poll_item_id)
	{		
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_content_poll_item_id").value	= content_poll_item_id;
	}
	else if(show=='0' && bid!=content_poll_item_id)
	{
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_content_poll_item_id").value	= content_poll_item_id;
	}
	else if(show=='1' && bid!=content_poll_item_id)
	{
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_content_poll_item_id").value	= content_poll_item_id;
	...
	//alert(show);
	var url="show_content_poll_item_details.php?content_poll_item_id="+content_poll_item_id+"&content_poll_item_serial_value="+serial_value+ "&show=" + show;
...
function select_all_content_poll_items(chkbx,frmobj)
{
...
function toggle_select_comment_all(chkbx,frmobj,content_poll_itemid)
{
		var frmObj=document.forms[frmobj]; 	
		var chkbxname='checkbox1_'+content_poll_itemid+'[]';	
		var count = document.getElementsByName(chkbxname).length; 
...
function validate_content_poll_item_search(frmobj)
{
	if((document.forms[frmobj].content_poll_item_category.value==0) &&  (trim(document.forms[frmobj].search_word.value) == ""))
	{ 
...
function delete_all_content_poll_items(frmName, field, exclbox)
{
...
function change_content_poll_item_status(content_poll_item_id, selected_id)
{
	if (content_poll_item_id.length == 0)
	{ 
		return;
	}
	...
	var url="change_content_poll_item_status.php";

	url = url+"?content_poll_item_id=" + content_poll_item_id;
	//alert(url);
	url = url+"&stid="+Math.random();
	xml_http_content.onreadystatechange	= change_content_poll_item_status_onready;
	xml_http_content.open("GET",url,true);
	xml_http_content.send(null);
} 
	
function change_content_poll_item_status_onready()
{ 
