// JavaScript Document
function open_planning_rating_details(planning_rating_id,divId, show_edit_comment, comment_id, serial_value)
{ 
 	var ajaxrequest;
	...
	if(document.getElementById("show_planning_rating_id").value==0) {  document.getElementById("show_planning_rating_id").value=planning_rating_id;}
	var bid		= document.getElementById("show_planning_rating_id").value;
	//alert(bid+"   "+planning_rating_id);
	if(show=='0' && bid==planning_rating_id)
	{		
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_planning_rating_id").value	= planning_rating_id;
	}
	else if(show=='0' && bid!=planning_rating_id)
	{
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_planning_rating_id").value	= planning_rating_id;
	}
	else if(show=='1' && bid!=planning_rating_id)
	{
		  show=1;
		  document.getElementById("show").value	= 1;
		  document.getElementById("show_planning_rating_id").value	= planning_rating_id;
	...
	//alert(show);
	var url="show_planning_rating_details.php?planning_rating_id="+planning_rating_id+"&artist_review_serial_value="+serial_value+ "&show=" + show;
...
{
	if((document.forms[frmobj].testimonial_category.value==0) &&  (trim(document.forms[frmobj].search_word.value) == ""))
	{ 
...
function change_artist_review_status(content_id, selected_id)
{
	...
	var url="change_artist_review_status.php";

	url = url+"?planning_rating_id=" + content_id;
	//alert(url);
