...
*********************************************************************************************/
    class news_letter
	{
        ...
        function __construct($news_letter_id = 0)
		{
            $this->error	= '';
			$this->message	= '';
			$this->warning	= false;
			...
			if($news_letter_id > 0)
			{
				$this->initialize($news_letter_id);
			}
        }
		...
		private function initialize($news_letter_id)
		{
			$database	= new database();
			$sql		= "SELECT *	 FROM news_letter WHERE news_letter_id = '$news_letter_id'";
		...
		public function check_news_letter_exist($email='')
		{
			$output		= false;
			$database	= new database();
	
			$sql	= "SELECT *	 FROM news_letter WHERE email = '" . $database->real_escape_string($email) . "'";
			
			$result 	= $database->query($sql);
			if ($result->num_rows > 0)
			{
				$this->message	= "You are already news_letter news letter!";
				$output 		= true;
			}
			else
			{
		...
		{			
			  $database	= new database();
			  if(!$this->check_news_letter_exist($this->email))
			  {
				   $sql		= "INSERT INTO news_letter ( email , active, date_subscription) 
								VALUES ('" . $database->real_escape_string($this->email) . "','1',NOW())";
								
				  $result			= $database->query($sql);
				  if($database->affected_rows == 1)
				  {
					  $mail_content	= '';
					  $email_template	= new email_template('newsletter.message');	
	...
	$sql 			= $sql . " ORDER BY news_letter_id DESC ";
		...
		{				
			...
			<tr class="lightColorRow nodrop nodrag" style="display:none;">
				<td colspan="7"  class="noBorder">
					...
					<input type="hidden" id="news_letter_id" name="news_letter_id" value="0" />
			...
			while($data=$result->fetch_object())
			{
				...
				$news_letter_of_month	= $data->news_letter_of_month == 'Y' ? '<img src="images/icon-active.png">' : '';
			$date='';  
			if($data->date_subscription!= '0000-00-00 00:00:00')
			{
			$date= functions::get_format_date($data->date_subscription,'d-m-Y');
			}
				
				echo '
					<tr id="' . $data->news_letter_id . '" class="' . $class_name . $drag_drop . '" >
...
<!--						<td class="alignCenter joiningDateCol"><a title="Click here to update status" class="handCursor" onclick="javascript: change_news_letter_status(\'' . $data->news_letter_id . '\', \'' . $i . '\');" ><img id="status_image_' . $i . '" src="images/' . $status_image . '" alt ="' . $status  . '" title ="' . $status  . '"></a></td>-->
						<!--<td class="alignCenter">
							<a href="register_news_letter.php?news_letter_id=' . $data->news_letter_id . '"><img src="images/icon-edit.png" alt="Edit" title="Edit" width="15" height="16" /></a>
						</td>-->
						<td class="alignCenter deleteCol">
							<label><input type="checkbox" name="checkbox[' . $data->news_letter_id . ']" id="checkbox" /></label>
						</td>
					</tr>';
				$row_type++;
			}
		...
		else
		{
			...
			if($currentPage>1)
			{
				...
				if($this->pager_param=="")
				{
					$urlQuery = 'manage_news_letter.php?page='.$currentPage;
				}
				else
				{
					$urlQuery = 'manage_news_letter.php?'.$this->pager_param1.'&page='.$currentPage;	
				}
		...
		public function remove_selected($news_letter_ids)
	{
		$database	= new database();
		if(count($news_letter_ids)>0)
		{		
			foreach($news_letter_ids as $news_letter_id)
			{
								
					
										
					$sql = "DELETE FROM news_letter  WHERE news_letter_id = '" . $news_letter_id . "'";
